...
describe("globalErrorHandler", () => {
  ...
  it("should return a 500 when database is failing", async () => {
    ...
    }; // Ecrase le comportement par défaut de prisma.level.create en lui faisant lever une exception grâce à un mock
    it.mock.method(logger, "error", () => {}); // On désactive le log d'erreur pour ne pas polluer la console

    // ACT
    const httpResponse = await authedRequester.post("/levels", {
      name: "Un level valide",
    });
